...
void solve()
{
    ...
    for (auto &i : v)
        cin >> i;
    pbds<ll, less_equal<ll>> A;
    for (ll i = 0; i < k; i++)
        A.insert(v[i]);
    auto prev = *A.find_by_order((k + 1) / 2 - 1);
    ll temp = 0;
    for (ll i = 0; i < k; i++)
        temp += abs(v[i] - prev);
    cout << temp << " ";
    for (ll i = 0; i < n - k; i++)
    {
        A.erase(A.find_by_order(A.order_of_key(v[i])));
        A.insert(v[i + k]);
        auto it = *A.find_by_order((k + 1) / 2 - 1);
        temp = temp + abs(it - v[i + k]) - abs(prev - v[i]);
        if (k % 2 == 0)
            temp = temp - (it - prev);
        prev = it;
        cout << temp << " ";
